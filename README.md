...
* **Chromatic Aberration Glass Surfaces**: Ported the open-source `<GlassSurface />` component into a responsive Vanilla JS visual engine. It creates dynamic inline SVG filters (using `feDisplacementMap`, `feColorMatrix`, and `feBlend` overlays) to render high-end refracted glass borders on article card titles. The displacement map dimensions automatically sync via a `ResizeObserver` on all screens.
